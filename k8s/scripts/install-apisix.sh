...
WEB_UPSTREAM="${WEB_NAME}:${WEB_PORT}"
...
export API_HOST ADMIN_HOST WEB_HOST GATEWAY_UPSTREAM BACKEND_UPSTREAM UI_UPSTREAM WEB_UPSTREAM
...
envsubst '${API_HOST} ${ADMIN_HOST} ${WEB_HOST} ${GATEWAY_UPSTREAM} ${BACKEND_UPSTREAM} ${UI_UPSTREAM} ${WEB_UPSTREAM} ${PUBLIC_RATE_LIMIT_COUNT} ${PUBLIC_RATE_LIMIT_WINDOW_SECONDS} ${PUBLIC_MAX_BODY_SIZE_BYTES} ${UPSTREAM_CONNECT_TIMEOUT_SECONDS} ${UPSTREAM_SEND_TIMEOUT_SECONDS} ${UPSTREAM_READ_TIMEOUT_SECONDS} ${ADMIN_ACCESS_PLUGINS}' \
